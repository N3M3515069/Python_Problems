...
def choose_best_sum(t, k, ls):
    # Generate all possible combinations of k distances from the ls list
    combinations_list = list(combinations(ls, k))
    ...
    for combo in combinations_list:
        combo_sum = sum(combo)  # Calculate the sum of the current combination

        # Update the maximum sum if the current sum is valid and greater
        if combo_sum <= t and combo_sum > max_sum:
            max_sum = combo_sum
